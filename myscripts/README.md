...
* [Системные возможности](#Системные-возможности)
  ...
  * [очистка буфера "read"](#Очистка-буфера-"read")
...
Скрипт содержит код прверки наличия программы и пример его использования.


### Очистка буфера "read"

**[clearBuf.sh](clearBuf.sh)** -- Однострочник для очистки буфера для ввода `read` в скриптах bash. Применяется в циклах, чтобы в буфере `read` не накапливались лишние символы. Можно обернуть в виде функции.

Очистка вызывается непосредственно перед `read` -- особенно в циклах, чтобы в буфере `read` не накапливались лишние символы.

Скрипт содержит код очистки и пример его использования.
